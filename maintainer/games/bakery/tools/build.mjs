...
const expectedActions = ['TraySelected', 'AddOne', 'RemoveOne', 'Double', 'Halve', 'AddModifier', 'SubtractModifier', 'MultiplyModifier', 'DivideModifier', 'EqualCheck', 'LessCheck', 'GreaterCheck'];
const expectedVariables = ['batch', 'modifierNumber', 'leftCakes', 'rightCakes', 'ready'];
...
export function validateCatalog(catalog) {
  if (catalog.catalog_id !== 'BAKERY-CUMULATIVE-STATES-V5' || catalog.states?.length !== 12 || !/^\/\/ Cake Factory\n?$/.test(catalog.starter_source)) throw Error('Bakery catalog must contain twelve constructions and the Cake Factory comment-only starter carrier.');
  ...
  if (JSON.stringify(catalog.variable_types) !== JSON.stringify({batch:'number',modifierNumber:'number',leftCakes:'number',rightCakes:'number',ready:'Boolean'})) throw Error('The catalog must declare the reviewed variable types.');
  const introduced = new Set();
  const counts = {'+':0, '-':0, '*':0, '/':0};
  for (const [index, state] of catalog.states.entries()) {
    if (state.through !== index + 1 || state.tutorial_step !== index + 1 || state.action !== expectedActions[index] || !/^(?:0[1-9]|1[0-2])-[a-z][a-z-]+$/.test(state.id)) throw Error(`Unexpected state identity/order: ${state.id}`);
  ...
  if (introduced.size !== 5 || Object.values(counts).some(n=>n!==2) || JSON.stringify(catalog.states.slice(9).map(s=>s.comparison)) !== JSON.stringify(expectedComparisons)) throw Error('Five variables, two authored uses per arithmetic operator and EQ/LT/GT comparisons are required.');
  if (!Array.isArray(catalog.experiments) || catalog.experiments.length !== 0) throw Error('V5 must not contain experiments.');
  return {states:12, experiments:0, variables:[...introduced], arithmetic_constructions:counts, comparison_constructions:{'==':1,'<':1,'>':1}, starter_comment_only:true, starter_executable_code:false};
}
...
export function buildBakery(options = {}) {
  ...
  if ((tutorial.match(/^```customts$/gm)||[]).length!==1 || (tutorial.match(/^```template$/gm)||[]).length!==1 || (tutorial.match(/^```blocks$/gm)||[]).length!==12) throw Error('Tutorial resource cardinality changed.');
